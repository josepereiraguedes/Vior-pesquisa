        ...
        {question.options?.map((opt) => {
          ...
          return (
            <button
              ...
              className={`flex items-center p-4 border-2 rounded-xl transition-all text-left shadow-sm ${isSelected
...
const TextView: React.FC<{ question: Question, onAnswer: (val: string) => void, isLast: boolean, isSubmitting?: boolean }> = ({ question, onAnswer, isLast, isSubmitting }) => {
  ...
  const handleChange = (e: React.ChangeEvent<HTMLTextAreaElement>) => {
    ...
    if (question.id === 'whatsapp') {
      ...
      if (val.length > 15) return; // (XX) XXXXX-XXXX is 15 chars
    }
    setValue(val);
  };
  ...
  return (
    <div className="w-full space-y-4">
      ...
      <button
        ...
        disabled={(question.required && !value.trim()) || isSubmitting}
...
export const SurveyFlow: React.FC<SurveyFlowProps> = ({ onComplete, isSubmitting }) => {
  const [currentIndex, setCurrentIndex] = useState(() => {
    const saved = localStorage.getItem('vior_survey_progress');
  ...
  const validateAnswer = (answer: string | string[] | number): string | null => {
    if (!currentQuestion.required) return null;

    if (currentQuestion.id === 'name') {
      const val = String(answer).trim();
      if (val.length < 3) return 'Por favor, insira seu nome completo (mínimo 3 letras).';
      if (/^(\w)\1+$/.test(val)) return 'Por favor, insira um nome válido.';
    }

    if (currentQuestion.id === 'whatsapp') {
      const val = String(answer).replace(/\D/g, '');
      if (val.length < 10) return 'Por favor, insira um número de WhatsApp válido.';
    }

    if (typeof answer === 'string' && !String(answer).trim()) return 'Por favor, responda esta pergunta.';
    if (Array.isArray(answer) && answer.length === 0) return 'Selecione pelo menos uma opção.';
  ...
  const handleNext = (answer: string | string[] | number) => {
    ...
    // Abandonment tracking / Progress saving
    localStorage.setItem('vior_survey_progress', JSON.stringify({
      currentIndex: isLastQuestion ? 0 : currentIndex + 1,
      responses: isLastQuestion ? [] : updatedResponses
    }));
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-pink-50 to-purple-50 flex flex-col items-center justify-center p-4">
      <div className="w-full max-w-2xl">
        ...
        {/* Question Card */}
        <div className="bg-white/80 backdrop-blur-sm p-6 sm:p-10 rounded-3xl shadow-xl shadow-purple-100/50 border border-white overflow-hidden">
              ...
              <div className="min-h-[200px] flex items-center justify-center">
                ...
                */}
                {currentQuestion.type === 'image_select' && (
                  <ImageSelectView question={currentQuestion} onAnswer={(v) => handleNext(v)} />
                )}
                {currentQuestion.type === 'single_choice' && (
                  <SingleChoiceView question={currentQuestion} onAnswer={(v) => handleNext(v)} />
                )}
                {currentQuestion.type === 'multiple_choice' && (
                  <MultipleChoiceView question={currentQuestion} onAnswer={(v) => handleNext(v)} isLast={isLastQuestion} />
                )}
                {currentQuestion.type === 'rating' && (
                  <RatingView question={currentQuestion} onAnswer={(v) => handleNext(v)} isLast={isLastQuestion} />
                )}
                {currentQuestion.type === 'text' && (
                  <TextView question={currentQuestion} onAnswer={(v) => handleNext(v)} isLast={isLastQuestion} isSubmitting={isSubmitting} />
                )}
              </div>
            </motion.div>
          </AnimatePresence>
